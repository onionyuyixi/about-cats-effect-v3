...
//import com.example.aboutcats.cats.threads.WorkStealingThreadPoolConstants._
//
///**
...
//    private var parked: AtomicBoolean,
...
//      callback: Right[Nothing, Unit] => Unit): (() => Unit) with Runnable = {
...
//   */
//  private def active_=(fiber: Runnable): Unit = {
//    _active = fiber
//  }
//
//  /**
...
//   */
//  private def suspendedTraces(): Map[Runnable, Trace] = {
...
//            if (otherIdx == idx) {
//              otherIdx =
//                (idx + Math.max(1, random.nextInt(pool.getWorkerThreads.length - 1))) %
//                  pool.getWorkerThreads.length
//            }
...
//            if (parked != null && !parked.get() && (state == Thread.State.BLOCKED || state == Thread
